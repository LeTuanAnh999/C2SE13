...
import { Sign } from '../url_page/url_page';
import { useNavigate } from "react-router-dom";
import logo from "../LOGO.png"
import { message} from 'antd';
import { SignCandidateapi } from '../../api/candidateapi/Candidateapi';
...
function User_login() {
    ...
    const onValueChangeEmail = (event) => {
        ...
        setdatalogin((prev) => ({ ...prev, email: event.target.value }));
 ...
 const onValueChangePassword = (event) => {
        setdatalogin((prev) => ({ ...prev, password: event.target.value }));
    };    const warning = () => {
        messageApi.open({
          type: 'warning',
    ...
    const hanldesumbitlogn =async ()=>{


        if(datalogin.email === '' || datalogin.password ==='' || isGmailAddress(datalogin.email) === false){
            warning();

        }else   if(isGmailAddress(datalogin.email) === true) {
            const data = new FormData();
            data.append("email", datalogin.email);
            data.append("password", datalogin.password)
            const result = await SignCandidateapi(data);
            // console.log(result)
            if(result.data.message ==="Mật khẩu sai"){
                messageApi.open({
                    type: 'warning',
            ...
            if(result.data.message !== 'lỗi' && result.data.message !=="Mật khẩu sai"){
                //setlocalstage
                window.localStorage.setItem("cccd",(result.data.message));
                navigate("/");
            }
        ...
        <div className="col-lg-6 auth-form">
            ...
            <div className="register">
                    ...
                    <div className="form-group mb-3">
                        <label htmlFor="fullname" className="mb-1">Tài khoản:</label>
                        <div className="input-group " id="validateemail">
                            <div className="input-group-prepend">
                                <span className="input-group-text"><i className="fa-solid fa-user"></i></span>
                            </div>
                            <input type="text" name="fullname" className="form-control"
                                placeholder="Nhập email đăng ký tài khoản" aria-label="Nhập email đăng ký tài khoản"
                        ...
                        <div className="input-group ">
                            ...
                            <div className="input-group-prepend">
                                <button type="button" tabIndex="-1" data-input-target="#password"
                                    className="input-group-text toggle-password"></button>
                            </div>
                        </div>
                    </div>
                    {/* <div className="form-group mb-3">
                    ...
                    <div className="form-group mt-3">
                        ...
                        <button type="submit"
                            ...
                            className="btn btn-sign input-block-level w-100 h-40 mb-3 btn-primary-hover" onClick={hanldesumbitlogn}>Đăng nhập</button>
                ...
                <div className="mt-3 d-flex justify-content-between option-auth">
                    <div  onClick={routeChange}>
                        ...
                        <a className="text-success font-weight-bold" href="/user/sign">
                            Đăng ký ngay
                        </a>
                    </div>
                </div>
                <div className="mt-4 support fz-12px">
                    ...
                    <p className="mb-0">
                        Vui lòng gọi tới số (84) 961903217  (giờ hành chính).
                    </p>
                </div>
            </div>
